...
export default function StadiumsSportGroundsClubsPage() {
  ...
  const loadChapter = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_stadiums_sport_grounds_clubs')
      const data = await response.json()
      ...
      if (response.ok) {
        ...
        const analyticsResponse = await fetch('/api/analytics/chapter/chapter_stadiums_sport_grounds_clubs')
  ...
  const handleStrategyConfirm = (strategy: 'mix' | 'new_only' | 'prioritize_new') => {
    ...
    router.push(`/dashboard/chapters/stadiums-sport-grounds-clubs/quiz?count=${count}&strategy=${strategy}`)
  }

  const handleStartChapter = () => {
    const count = questionCount === 'all' ? 'all' : questionCount
    router.push(`/dashboard/chapters/stadiums-sport-grounds-clubs/quiz?count=${count}&strategy=mix`)
  }

  const totalQuestions = chapter?.totalQuestions || 64
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50">
        ...
        <div className="px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Stadiums, Sport Grounds and Clubs</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-6 max-w-4xl mx-auto pb-20">
        {chapter && (
          <>
            ...
            <ChapterModeButton chapterSlug="stadiums-sport-grounds-clubs" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {hasStarted && (
                <Link
                  href="/dashboard/chapters/stadiums-sport-grounds-clubs/analytics"
      ...
      <QuestionSelectionModal
        ...
        totalQuestions={chapter?.totalQuestions || 64}
