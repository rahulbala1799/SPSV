...
export default function SouthsideFullChapterPage() {
  ...
  const loadChapter = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_tourist_attractions')
      const data = await response.json()
      ...
      if (response.ok) {
        ...
        const analyticsResponse = await fetch('/api/analytics/chapter/chapter_tourist_attractions')
  ...
  const handleStrategyConfirm = (strategy: 'mix' | 'new_only' | 'prioritize_new') => {
    ...
    router.push(`/dashboard/chapters/tourist-attractions/quiz?count=${count}&strategy=${strategy}`)
  }

  const handleStartChapter = () => {
    // Use current question count with mix strategy (default)
    const count = questionCount === 'all' ? 'all' : questionCount
    router.push(`/dashboard/chapters/tourist-attractions/quiz?count=${count}&strategy=mix`)
  }

  const totalQuestions = chapter?.totalQuestions || 62
        ...
        <div className="px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Tourist Attractions & Landmarks</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-6 max-w-4xl mx-auto pb-20">
        {chapter && (
          <>
            ...
            <ChapterModeButton chapterSlug="tourist-attractions" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {hasStarted && (
                <Link
                  href="/dashboard/chapters/tourist-attractions/analytics"
