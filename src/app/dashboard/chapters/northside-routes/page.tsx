...
export default function SouthsideFullChapterPage() {
  ...
  const loadChapter = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_northside_routes')
      const data = await response.json()
      ...
      if (response.ok) {
        ...
        const analyticsResponse = await fetch('/api/analytics/chapter/chapter_northside_routes')
  ...
  const handleStrategyConfirm = (strategy: 'mix' | 'new_only' | 'prioritize_new') => {
    ...
    router.push(`/dashboard/chapters/northside-routes/quiz?count=${count}&strategy=${strategy}`)
  }

  const handleStartChapter = () => {
    // Use current question count with mix strategy (default)
    const count = questionCount === 'all' ? 'all' : questionCount
    router.push(`/dashboard/chapters/northside-routes/quiz?count=${count}&strategy=mix`)
  }

  const totalQuestions = chapter?.totalQuestions || 113
        ...
        <div className="px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Northside Routes</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-6 max-w-4xl mx-auto pb-20">
        {chapter && (
          <>
            ...
            <ChapterModeButton chapterSlug="northside-routes" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {hasStarted && (
                <Link
                  href="/dashboard/chapters/northside-routes/analytics"
