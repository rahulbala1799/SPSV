...
export default function DublinOneWayStreetsChapterPage() {
  ...
  const loadChapter = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_dublin_one_way_streets')
      const data = await response.json()
      ...
      if (response.ok) {
        ...
        const analyticsResponse = await fetch('/api/analytics/chapter/chapter_dublin_one_way_streets')
  ...
  const handleStrategyConfirm = (strategy: 'mix' | 'new_only' | 'prioritize_new') => {
    ...
    router.push(`/dashboard/chapters/dublin-one-way-streets/quiz?count=${count}&strategy=${strategy}`)
  }

  const handleStartChapter = () => {
    // Use current question count with mix strategy (default)
    const count = questionCount === 'all' ? 'all' : questionCount
    router.push(`/dashboard/chapters/dublin-one-way-streets/quiz?count=${count}&strategy=mix`)
  }

  const totalQuestions = chapter?.totalQuestions || 27
  const questionCountOptions: Array<{ value: number | 'all'; label: string }> = [
    { value: 5, label: '5 Questions' },
    { value: 10, label: '10 Questions' },
    { value: 15, label: '15 Questions' },
    { value: 20, label: '20 Questions' },
    { value: 27, label: '27 Questions' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50">
        ...
        <div className="px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Dublin One Way Streets</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-6 max-w-4xl mx-auto pb-20">
        {chapter && (
          <>
            ...
            <ChapterModeButton chapterSlug="dublin-one-way-streets" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {hasStarted && (
                <Link
                  href="/dashboard/chapters/dublin-one-way-streets/analytics"
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3"
                >
      ...
      <QuestionSelectionModal
        ...
        totalQuestions={chapter?.totalQuestions || 27}
