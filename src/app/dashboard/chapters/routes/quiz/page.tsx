...
function QuizContent() {
  ...
  const loadQuestions = async () => {
    try {
      ...
      const response = await fetch(`/api/chapters/chapter_routes/questions?${queryParams.toString()}`)
      const data = await response.json()
  ...
  const loadProgress = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_routes/progress')
      const data = await response.json()
  ...
  const handleSubmitAnswer = async (questionId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/chapters/chapter_routes/questions/${questionId}/answer`,
        {
          method: 'POST',
  ...
  const handleCompleteChapter = async () => {
    ...
    router.push(`/dashboard/chapters/routes/results?${params.toString()}`)
  }
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40">
        <div className="px-4 py-4">
          <div className="flex items-center justify-between">
            <Link
              href="/dashboard/chapters/routes"
