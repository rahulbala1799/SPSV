...
export default function SouthsideFullChapterPage() {
  ...
  const loadChapter = async () => {
    try {
      const response = await fetch('/api/chapters/chapter_routes')
      const data = await response.json()
      ...
      if (response.ok) {
        ...
        const analyticsResponse = await fetch('/api/analytics/chapter/chapter_routes')
  ...
  const handleStrategyConfirm = (strategy: 'mix' | 'new_only' | 'prioritize_new') => {
    ...
    router.push(`/dashboard/chapters/routes/quiz?count=${count}&strategy=${strategy}`)
  }

  const handleStartChapter = () => {
    // Use current question count with mix strategy (default)
    const count = questionCount === 'all' ? 'all' : questionCount
    router.push(`/dashboard/chapters/routes/quiz?count=${count}&strategy=mix`)
  }

  const totalQuestions = chapter?.totalQuestions || 61
        ...
        {chapter && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {hasStarted && (
                <Link
                  href="/dashboard/chapters/routes/analytics"
